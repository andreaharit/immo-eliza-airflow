...
from src_deploy.plots import Plots
...
def deploy(clean_data, encoder,scaler,model):
...
"Veurne", "Virton", "Waremme"]
    ...
    select_epc = st.select_slider(label ="What is the house's EPC?", options = list(group_epc.keys()), value = "A")
    ...
    if st.button(label = "Predict Price", use_container_width = True, type="primary"):
        ...
        predict = Predict(data=test, encoder=encoder, scaler=scaler, model=model)
        prediction = int(predict.result)
        # Prints the result from the API    
        st.success(f"The predicted price for your house is €{prediction}.")
    ...
    """.format(district=district)
        ...
        plots = Plots(file = clean_data, district = district )
        ...
        with st.container():
            ...
            with tab1:
                ...
                price_plot = plots.price_sqm(prediction = prediction, living_area = living_area)                
...
if __name__ == "__main__":
    deploy(clean_data = latest_clean, encoder = encoder, scaler = scaler, model = latest_model)
